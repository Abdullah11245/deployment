...
function Receiptreport() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(partiesRes.data)

        const voucherData = (voucherRes.data || []).filter(v => v.voucher_type === 'SV');
        ...
        const partyOptionsFromMap = Object.keys(partyNameMap)
  .filter(code => partyNameMap[code] !== 'Unknown') // Add this filter
  .map(code => ({
    value: code,
    label: partyNameMap[code],
  }));

        setPartiesOptions(partyOptionsFromMap);
  ...
  const getPaginatedData = () => {
    ...
    return validRoutes.slice(indexOfFirstItem, indexOfLastItem);
  };
  ...
  const handleSearch = () => {
    ...
    const filtered = voucherData.filter(voucher => {
      ...
      const supplierIncluded =
        includedSupplierIds.length === 0 || supplierCodes.some(code => includedSupplierIds.includes(code));
      const supplierExcluded =
        excludedSupplierIds.length === 0 || supplierCodes.every(code => !excludedSupplierIds.includes(code));
      ...
      return isWithinDate && supplierIncluded && supplierExcluded && hasKnownParty && hasKnownRoute;
    });
  ...
  const handleReset = () => {
    ...
    // Apply default filtered vouchers (removing Unknown/N/A)
    const cleanedData = voucherData.filter(voucher => {
      const details = getDetailsForVoucher(voucher.id);
...
const exportPDF = () => {
      ...
      <table className="min-w-full table-auto border-collapse">
        <thead>
          <tr className="text-sm font-semibold bg-gray-100">
            ...
            <th className='py-3 px-4 text-left'>Party Code</th>
  ...
  {(() => {
    const details = getDetailsForVoucher(route.id);
    const matchedParty = details.find(detail =>detail.account_code);
    return matchedParty ? matchedParty.account_code: 'N/A';
  })()}
</td>
                <td className="py-3 px-4">
  {(() => {
