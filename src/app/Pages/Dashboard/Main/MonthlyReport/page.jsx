...
const getCurrentBalance = (voucherId) => {
  const details = getDetailsForVoucher(voucherId);
  return getTotalAmount(details); // Returns debit - credit
};

const getPreviousBalance = (voucherId) => {
  const currentBalance = getCurrentBalance(voucherId);
  const lastPaid = getLastPaidIn(voucherId);
  const lastPaidAmount = lastPaid ? parseFloat(lastPaid.debit) : 0;
  return currentBalance - lastPaidAmount;
};
...
const exportPDF = () => {
  ...
  const getLastPaidIn = (voucherId) => {
    const details = getDetailsForVoucher(voucherId);
    const debitEntries = details.filter(detail => parseFloat(detail.debit) > 0);
    if (debitEntries.length === 0) return null;
    return debitEntries[debitEntries.length - 1];
  };
      ...
      <table className="min-w-full table-auto border-collapse">
        <thead>
          <tr className="text-sm font-semibold bg-gray-100">
            ...
            <th className="py-3 px-4 text-left">Current Balance</th>
            <th className="py-3 px-4 text-left">Previous Balance</th>
  ...
  {(() => {
    const details = getDetailsForVoucher(route.id);
    const matchedParty = details.find(detail => routeNameMap[detail.account_code]);
  ...
  {(() => {
    ...
    if (matchedParty) {
      ...
      return 'N/A';
               ...
               <td className="py-3 px-4">
  {(() => {
    const lastPaid = getLastPaidIn(route.id);
    return lastPaid ? formatAmountToPKR(parseFloat(lastPaid.debit)) : '0';
  })()}
</td>
...
<td className="py-3 px-4">{formatAmountToPKR(getPreviousBalance(route.id))}</td>
<td className="py-3 px-4">{formatAmountToPKR(getCurrentBalance(route.id))}</td>
